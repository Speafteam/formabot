...
# Ключи FatSecret гасим только у файла, принесённого с компьютера: пока IP
# сервера не в белом списке, заполненные ключи дают отказ на каждую запись еды,
# а пустые — понятное сообщение. Уже настроенный на сервере .env не трогаем,
# иначе повторный запуск скрипта стирал бы рабочие ключи.
if [ "$NEED_ENV" = "0" ] && [ ! -f "$DIR/.env.configured" ] \
   && [ "${KEEP_FATSECRET:-0}" != "1" ]; then
    sed -i 's/^\(FATSECRET_[A-Z_]*\)=.*/\1=/' "$DIR/.env"
    touch "$DIR/.env.configured"
fi
